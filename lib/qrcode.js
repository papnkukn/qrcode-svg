...
QRCode.prototype.svg = function(opt) {
  ...
  var path = '<path x="0" y="0" style="fill:' + options.color + ';shape-rendering:crispEdges;" d="'
  var dataPath = ''
  for (var y = 0; y < length; y++) {
    for (var x = 0; x < length; x++) {
      var module = modules[x][y];
      if (module) {
        var px = (y * ysize + options.padding * ysize);
        var py = (x * xsize + options.padding * xsize);
        var w = xsize + px
        var h = ysize + py
        ...
        path += ('M' + px + ',' + py + ' V' + h + ' H' + w + ' V' + py + ' H' + px + ' Z ')
      }
    }
  }
  path += '" />'

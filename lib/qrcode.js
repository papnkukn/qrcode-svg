...
/** Generates QR Code as HTML table image */
QRCode.prototype.table = function(opt) {
  ...
  for (let row = 0; row < nCount; row++) {
    ...
    for (let col = 0; col < nCount; col++) {
      ...
      aHTML.push('<td style="border:0;border-collapse:collapse;padding:0;margin:0;width:' + nWidth + 'px;height:' + nHeight + 'px;background-color:' + (module ? options.color : options.background) + ';"></td>');
    }
    aHTML.push('</tr>');
  }

  aHTML.push('</table>');
  const innerHtml = aHTML.join('');
  return innerHtml;
}
